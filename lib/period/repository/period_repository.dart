...
class PeriodRepository {
  ...
  Future init() async {
    _database = await openDatabase(
      ...
      onCreate: (db, version) {
        ...
        return db.execute('CREATE TABLE $tableName($idColumn INTEGER PRIMARY KEY, '
            '$dateColumn INTEGER, '
  ...
  Future<List<DateTime>> getPeriodDates() async {
    final List<Map<String, dynamic>> maps =
        await _database.query(tableName, where: '$hadPeriodColumn = ?', whereArgs: [1]);
  ...
  Future<DailyInfo> getInfoForDate(DateTime date, double defaultTemperature, bool defaultBirthControl) async {
    final List<Map<String, dynamic>> entry =
        await _database.query(tableName, where: '$dateColumn = ?', whereArgs: [date.millisecondsSinceEpoch], limit: 1);
  ...
  Future<void> updateInfoForDay(DailyInfo dailyInfo) async {
    await _database.update(tableName, dailyInfo.toMap(),
        where: '$idColumn = ?', whereArgs: [dailyInfo.id], conflictAlgorithm: ConflictAlgorithm.replace);
  }
